...
parser = argparse.ArgumentParser(
    description="Sequence Modeling - (Permuted) Sequential MNIST"
)
parser.add_argument(
    "--batch_size", type=int, default=64, metavar="N", help="batch size (default: 64)"
)
...
parser.add_argument(
    "--epochs", type=int, default=20, help="upper epoch limit (default: 20)"
)
...
parser.add_argument(
    "--lr", type=float, default=2e-3, help="initial learning rate (default: 2e-3)"
)
parser.add_argument(
...
parser.add_argument(
    "--permute", action="store_true", help="use permuted MNIST (default: false)"
)
...
def train(ep):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx > 0 and batch_idx % args.log_interval == 0:
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tSteps: {}".format(
                    ep,
